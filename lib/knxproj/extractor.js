...
async function openZipBuffer(buffer, password) {
    ...
    // This avoids loading ALL files into memory upfront (saves hundreds of MB
    // on large projects with many manufacturer application XMLs).
    const files = [];
        ...
        files.push({
            ...
            buffer: async function () {
                ...
                try {
                    ...
                    this._entry = null; // free zip entry reference
                    return this._buf;
                } catch (err) {
                    if (err.message && (err.message.includes("password") || err.message.includes("Password"))) {
    ...
    // Reader will be garbage-collected when all file references are released
