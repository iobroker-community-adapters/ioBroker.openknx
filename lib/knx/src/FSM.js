...
const ResponseEventName = '_confirmed_';
...
module.exports = machina.Fsm.extend({
  initialize(options) {
    ...
    // FIX: Track pending send timeouts for cleanup on disconnect/reconnect
    this.pendingSendTimeouts = new Set();
  ...
  states: {
    ...
    connecting: {
      _onEnter() {
        ...
        // FIX: Clear all pending send timeouts on reconnection to prevent stale callbacks
        if (this.pendingSendTimeouts && this.pendingSendTimeouts.size > 0) {
          this.log.debug('Clearing %d pending send timeouts on reconnection', this.pendingSendTimeouts.size);
          for (const timeoutId of this.pendingSendTimeouts) {
            clearTimeout(timeoutId);
          }
          this.pendingSendTimeouts.clear();
        }
        // Reset queue length on reconnection
        this.queueLength = 0;
          ...
          this.connecttimer = setInterval(() => {
            ...
            if (connection_attempts >= 3) {
              ...
              if (this.remoteEndpoint.addr.range() == 'multicast') {
                ...
                this.log.info('reattempting connection in ' + delay + ' seconds');
                // also close the socket for failsave reasons and reopen after delay time
                try {
                  this.socket.close();
                } catch (error) {}
                setTimeout( () =>  {
      ...
      inbound_CONNECT_RESPONSE(datagram) {
        ...
        if (
          ...
          this.transition('uninitialized');
          this.emit('disconnected');
          this.log.warn('The KNXnet/IP server rejected the data connection (Maximum connections reached). Waiting 1 minute before retrying...');
          setTimeout(() => {
            this.Connect();
          }, 60000);
        } else {
    ...
    connected: {
      _onEnter() {
        ...
        // DEBUG: Log listener counts to detect potential leaks
        const transitionListeners = this.listeners ? this.listeners('transition').length : 'N/A';
        const disconnectedListeners = this.listeners ? this.listeners('disconnected').length : 'N/A';
        this.log.debug('Listener counts - transition: %s, disconnected: %s, pendingTimeouts: %d',
          transitionListeners, disconnectedListeners, this.pendingSendTimeouts ? this.pendingSendTimeouts.size : 0);
    ...
    idle: {
      ...
      _onExit() {
        // FIX: Clear idle timer when leaving idle state to prevent stale callbacks
        clearTimeout(this.idletimer);
        this.idletimer = null;
      },
      ...
      outbound_ROUTING_INDICATION(datagram, callback) {
        const request = 'outbound_ROUTING_INDICATION';
        if (typeof this.queueLength == 'undefined') {
          this.queueLength = 0;
        }
        if (typeof datagram.disenqueue == 'undefined') {
          datagram.disenqueue = false;
        }
        const elapsed = Date.now() - this.lastSentTime;
        // if no miminum delay set OR the last sent datagram was long enough ago OR delayed datagram waited long enough..
        if (!this.options.minimumDelay ||
          (elapsed >= this.options.minimumDelay && this.queueLength == 0) ||
          datagram.disenqueue) {
            /// register caller for notifications as it is sent out
          ...
          // .. or else, let the FSM handle it later after queue is processed
          var timeToWait = this.options.minimumDelay - elapsed;
          if (timeToWait < 0)
            timeToWait = 0;
          timeToWait += this.queueLength * this.options.minimumDelay;
          this.queueLength += 1;
          datagram.disenqueue = true;
          // FIX: Track timeout for cleanup on disconnect
          const timeoutId = setTimeout(() => {
            this.pendingSendTimeouts.delete(timeoutId);
            this.handle(request, datagram, callback);
          }, timeToWait);
          this.pendingSendTimeouts.add(timeoutId);
          KnxLog.get().debug('(%s): Send request delayed by %sms to respect minimum delay, queue length is %s, pending timeouts: %d',
            this.compositeState(), timeToWait, this.queueLength, this.pendingSendTimeouts.size);
        }
      },

      // 2) queue an OUTGOING tunelling request...
      outbound_TUNNELING_REQUEST(datagram, callback) {
        if (this.useTunneling) {
          const request = 'outbound_TUNNELING_REQUEST';
          if (typeof this.queueLength == 'undefined') {
            this.queueLength = 0;
          }
          if (typeof datagram.disenqueue == 'undefined') {
      ...
      /* 4) receive an INBOUND tunneling request CONFIRMATION (L_Data.con) to one of our sent tunnreq's (as response to our L_Data.req)
      ...
      ['inbound_TUNNELING_REQUEST_L_Data.con'](datagram) {
        if (this.useTunneling) {
          const confirmed = this.sentTunnRequests[datagram.cemi.dest_addr] && ! /*confirmation error*/ datagram.cemi.ctrl.confirm;
          ...
          this.emit(
            util.format('%s' + ResponseEventName + '%s', evtName, datagram.cemi.dest_addr), 
            confirmed
          );
    ...
    // if idle for too long 60s, request connection state from the KNX IP router
    requestingConnState: {
      _onEnter() {
    ...
    sendTunnReq_waitACK: {
      _onEnter(datagram) {
        this.checkTimeout(datagram);
      },
      _onExit() {
        this.ackresent = false;
  ...
  checkTimeout(datagram) {
        //this.log.debug('setting up tunnreq timeout for %j', datagram);
        if (this.ackresent != true) {
          this.tunnelingAckTimer = setTimeout(() => {
            this.log.debug('timed out waiting for TUNNELING_ACK - retry');
            /*
            * TUNNELLING_REQUEST-Frames müssen mithilfe von TUNNELLING_ACK-Frames bestätigt werden.
            * Wenn ein TUNNELLING_REQUEST-Frame nicht innerhalb der TUNNELLING_REQUEST_TIME_OUT-Zeit
            * von einer (1) s bestätigt wird, muss das Telegramm vom sendenden KNXnet/IP-Gerät ein weiteres Mal mit
            * demselben Folgezählerwert gesendet werden.
            * Wenn das KNXnet/IP-Gerät einen TUNNELLING_ACK-Frame nicht innerhalb von
            * DEVICE_CONFIGURATION_REQUEST_TIMEOUT (= 1 s) empfängt oder der Status eines empfangenen
            * TUNNELLING_ACK-Frames irgendeinen Fehlerzustand meldete, muss das sendende Gerät den
            * TUNNELLING_REQUEST-Frame ein weiteres Mal senden und dann die Verbindung beenden, indem es
            * einen DISCONNECT_REQUEST-Frame an den Kontrollendpunkt des anderen Gerätes sendet.
            */
            this.send(datagram, (err) => {
              if (err) {
                this.transition('idle');
          ...
          this.log.debug('timed out waiting for TUNNELING_ACK - abort');
          this.transition('connecting');
...
module.exports.ResponseEventName = ResponseEventName;
