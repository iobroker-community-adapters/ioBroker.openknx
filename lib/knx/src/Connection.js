...
FSM.prototype.onUdpSocketMessage = function(msg, rinfo, callback) {
  ...
  try {
    ...
    KnxLog.get().error('(%s): Incomplete/unparseable UDP packet: %s: %s',
...
FSM.prototype.AddCEMI = function(datagram, msgcode) {
  ...
  datagram.cemi = {
    ...
    ctrl: {
      ...
      acknowledge: sendAck ? 1 : 0, // 0 don't care
      confirm: 0,                   // must be 0 for L_Data.req
      // 2nd byte
...
FSM.prototype.Request = function(type, datagram_template, callback) {
  ...
  this.handle('outbound_' + st, datagram, callback);
}
...
FSM.prototype.read = function(grpaddr, callback) {
  ...
  const self = this;
    ...
    function(){ sendConfNotificationRead(callback, grpaddr, self) }
  );
}

/*
send a respond request to the bus
a respond can not notify the caller
*/
FSM.prototype.respond = function(grpaddr, value, dptid) {
  if (grpaddr == null || value == null) {
    KnxLog.get().warn('respond: You must supply both grpaddr and value!');
    return;
  }
  const serviceType = this.useTunneling ?
    KnxConstants.SERVICE_TYPE.TUNNELING_REQUEST :
...
/*
send a write request to the bus
you can pass a callback function which gets bound to the RESPONSE datagram event
*/
FSM.prototype.write = function(grpaddr, value, dptid, callback) {
  ...
  try {
    ...
    // register for bus event and notifiy caller todo
    function(){ sendConfNotificationWrite(callback, grpaddr, self) })
  } catch (e) {
    KnxLog.get().warn(e);
  }
}
...
FSM.prototype.writeRaw = function(grpaddr, value, bitlength, callback) {
  if (grpaddr == null || value == null) {
    KnxLog.get().warn('writeRaw: You must supply both grpaddr and value!');
    return;
  }
...
FSM.prototype.Disconnect = function(cb) {
  ...
  this.onIdle(function() {
    ...
    // FIX: Use a named handler that removes itself after firing
    // This prevents listener accumulation on repeated disconnect/reconnect cycles
    const disconnectHandler = () => {
      KnxLog.get().debug('Disconnected from KNX');
      that.off('disconnected', disconnectHandler); // Remove listener after firing
      if(cb) {
        cb()
      }
    };
    that.on('disconnected', disconnectHandler);
    KnxLog.get().debug('Disconnect: Added one-time disconnected listener');
...
FSM.prototype.onIdle = function(cb) {
  if(this.state === 'idle') {
    ...
    // FIX: Use a named handler that removes itself after firing once
    // This prevents listener accumulation that causes progressive slowdown
    const handler = (data) => {
      if(data.toState === 'idle') {
        KnxLog.get().trace('Connection just transitioned to Idle');
        this.off("transition", handler); // Remove listener after firing
        cb();
      }
    };
    this.on("transition", handler);
    KnxLog.get().debug('onIdle: Added one-time transition listener (current count: %d)',
      this.listeners ? this.listeners('transition').length : 'unknown');
  }
}
...
function sendConfNotificationWrite(callback, grpaddr, fsm) {
  if (typeof callback == 'function') {
    let timer;
    // when the confirmation arrives:
    const responseEvent = 'GroupValue_Write' + FSM.ResponseEventName + grpaddr;
    ...
    const binding = (confirmed) => {
      ...
      // fire the callback
      callback(grpaddr, confirmed, false);
    }
    ...
    timer = setTimeout( () => {
      fsm.off(responseEvent, binding);
      callback(grpaddr, false, true);
    }, 3000);
  }
}

// notify sending application if timeout or if confirmation received
function sendConfNotificationRead(callback, grpaddr, fsm) {
  if (typeof callback == 'function') {
    ...
    setTimeout( () => fsm.off(responseEvent, binding), 3000);
  }
}
