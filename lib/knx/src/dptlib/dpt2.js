...
exports.formatAPDU = (value) => {
  if (value == null) 
    return log.error('Conversion error DPT2: cannot write null value');
  ...
  return log.error('Conversion error DPT2: Must supply an value {priority:<bool>, data:<bool>}');
};

exports.fromBuffer = (buf) => {
  if (buf.length !== 1) 
    return log.error('Conversion error DPT2: Buffer should be 1 byte long');
