...
exports.formatAPDU = (value) => {
  if (value == null) 
    return log.error('Conversion error DPT4: cannot write null value');

  if (typeof value !== 'string')
    return log.error('Conversion error DPT4: Must supply a character or string');

  const apdu_data = value.charCodeAt(0);
  if (apdu_data > 255) 
    return log.error('Conversion error DPT4: must supply an ASCII character');
...
exports.fromBuffer = (buf) => {
  if (buf.length != 1) 
    return log.error('Conversion error DPT4: Buffer should be 1 byte long');
