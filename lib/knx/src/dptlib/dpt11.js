...
exports.formatAPDU = (value) => {
  if (value == null) 
    return log.error('Conversion error DPT11: cannot write null value for DPT11');
  switch (typeof value) {
    case 'string':
    case 'number':
  ...
  if (isNaN(value.getDate()))
    return log.error('Conversion error DPT11: Must supply a numeric timestamp, Date or String object for DPT11 Date');
...
exports.fromBuffer = (buf) => {
  if (buf.length != 3) 
    return log.error('Conversion error DPT11: Buffer should be 3 bytes long');
  ...
  if (
    ...
    return log.error('Conversion error DPT11: %j => %d/%d/%d is not valid date according to DPT11',
      buf,
      day,
      month,
      year
