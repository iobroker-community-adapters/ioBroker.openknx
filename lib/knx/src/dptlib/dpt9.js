...
exports.formatAPDU = (value) => {
  if (!isFinite(value))
    return log.warn('Conversion error DPT9: cannot write non-numeric or undefined value');
...
exports.fromBuffer = (buf) => {
  if (buf.length != 2)
    return log.warn('Conversion error DPT9: buf should be 2 bytes long (got %d bytes)',
      buf.length
    );
