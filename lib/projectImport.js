            ...
            if (regex.test(statusElement)) {
                ...
                if (similarity(a, b) > adapter.config.regexSimil) {
                    ...
                    if (gaList.getDpById(statusElement).dptid  != gaList.getDpById(nonStatusElement).dptid) {
                        adapter.log.warn(gaList.getDpById(statusElement).dptid + " does not match " + gaList.getDpById(nonStatusElement).dptid) + ", consider defining a conversion function in the alias object";
                    }
                }
            }
        }
