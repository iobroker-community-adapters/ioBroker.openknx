...
module.exports = {
    ...
    parseInput: function (adapter, xml, callback) {
        ...
        if (xml) {
            ...
            adapter.log.debug("Parsed xml successfully");
        }
    ...
    generateIoBObjects: function (adapter, fileObjectList, callback) {
            ...
            Object.entries(xmlGroupAddresses).forEach(([, xmlGroupAddress]) => {
                ...
                if (dpt == "") {
                    const text = `Not adding ${fullPathName} because no DPT is assigned to GA ${xmlGroupAddress.getAttribute(
                        "Address",
                    )}. The GA is not connected to a device communication object in the ETS.`;
                    adapter.log.warn(text);
                    if (!err) {
                        err = text;
                    } else {
                ...
                if (dpt.indexOf(".") === -1) {
                    const text = `${fullPathName} has only set base DPT ${dpt} in ETS. No additional informations (e.g. unit) available for this GA.`;
                ...
                try {
                    ...
                    // allowed state, unresolvable dpt means raw interface
                }
        ...
        // add the name of middle group and main group to fullPath for two level and three level style projects
