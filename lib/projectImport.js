    ...
    generateIoBObjects: function (adapter, fileObjectList, callback) {
        ...
        function processFile(adapter, err, objectList) {
            ...
            Object.entries(groupAddresses).forEach(([key, groupAddress]) => {
                ...
                let autoread = true;
                let dptObj = {};
                let fullPath = expandPath(groupAddress, tools.formatGaForIob(gaName));;
                let range = [];
                let role = "state";
                let type;
                ...
                if (dpt.indexOf(".") === -1) {
                    const text = `${fullPath} has only set a base DPT in ETS. No additional information (e.g. unit) for this GA available.`;
                ...
                ({
                    ...
                    type,
                    role
                } = identifyRageRoleType(dptObj, range, dpt, type, role));
        ...
        function identifyRageRoleType(dptObj, range, dpt, type, role) {
        ...
        for (const statusElement of gaList) {
            if (regex.test(statusElement)) {
                ...
                if (similarity(a, b) > adapter.config.regexSimil) {
                    ...
                    if (gaList.getDataById(statusElement).native.dpt != gaList.getDataById(nonStatusElement).native.dpt) {
                        adapter.log.warn("create aliases: " + gaList.getDataById(statusElement).native.dpt + " does not match " + gaList.getDataById(nonStatusElement).native.dpt) + ", consider defining a conversion function in the alias object";
                    }
                }
            }
        }
