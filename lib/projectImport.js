...
module.exports = {
    ...
    generateIoBObjects: function (adapter, fileObjectList, callback) {
        ...
        function processFile(adapter, err, objectList) {
            ...
            if (adapter.config.statusRegex) {
                findStatusGAs(adapter, iobrokerObjects);
            }
...
function findStatusGAs(adapter, iobrokerObjects) {
    const regex = RegExp(adapter.config.statusRegex);
    iobrokerObjects.forEach((iobrokerObject) => {
        const name = iobrokerObject.common.name;
        if (regex.test(name.toLowerCase())) {
            const expression = name.toLowerCase().replace(regex, "");
            const maxElement = iobrokerObjects.reduce((previousValue, currentValue) => {
                if (!previousValue) {
                    return currentValue;
                }
                ...
                if (similarity(currentValue.common.name.toLowerCase(), expression) > similarity(previousValue.common.name.toLowerCase(), expression)) {
                    return currentValue;
                } else {
                    return previousValue;
                }
            ...
            if (similarity(expression.replace(" ", ""), maxElement.common.name.toLowerCase().replace(" ", "")) > 0.9) {
                maxElement.native.statusGA = iobrokerObject.native.address;
                iobrokerObject.native.actGA = maxElement.native.address;
            }
        }
    });
}
