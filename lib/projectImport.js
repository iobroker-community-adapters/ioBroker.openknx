...
module.exports = {
    ...
    async findStatusGAs(adapter, gaList,isAliasCreation, callback) {
        let count = 0;
            ...
            if (regex.test(statusElement)) {
                ...
                if (similarity(a, b) > adapter.config.regexSimil) {
                    ...
                    //found a match that is not and status and good enough, generate alias object
                        ...
                        //deprecated direct actor and status connection like knx adapter
                        const nonStatusElementObject = await adapter.getObjectAsync(nonStatusElement);
                        const statusElementObject = await adapter.getObjectAsync(statusElement);
                        if (nonStatusElementObject && statusElementObject &&  statusElementObject.native && nonStatusElementObject.native) {
                            nonStatusElementObject.native.statusGA = statusElementObject.native.address;
                            statusElementObject.native.actGA = nonStatusElementObject.native.address;
                            await adapter.setObjectAsync(statusElement, statusElementObject);
                            await adapter.setObjectAsync(nonStatusElement, nonStatusElementObject);
                            adapter.log.info("create connection: found match " + nonStatusElement + " and " + statusElement);
                        }
                    }
                    count++;
                    adapter.log.info("create aliases: found match " + nonStatusElement + " and " + statusElement);
                    if (gaList.getDpById(statusElement).dptid  != gaList.getDpById(nonStatusElement).dptid) {
                        adapter.log.warn(gaList.getDpById(statusElement).dptid + " does not match " + gaList.getDpById(nonStatusElement).dptid);
                    }
