...
module.exports = {
    ...
    parseInput: function (adapter, xml, addNoDPTObjects, callback) {
        ...
        this.generateIoBObjects(adapter, fileObjectList, addNoDPTObjects, (error, result) => {
            callback(error, result);
        });
    },

    // generate the Groupaddress structure
    generateIoBObjects: function (adapter, fileObjectList, addNoDPTObjects, callback) {
        const groupAddresses = {};
        const iobrokerObjects = [];

        function processFile(adapter, addNoDPTObjects, err, objectList) {
            //from xml export
        ...
        if (!Object.prototype.hasOwnProperty.call(fileObjectList, "push")) {
            processFile(adapter, addNoDPTObjects, null, fileObjectList);
        }
    },
