...
module.exports = {
    ...
    generateIoBObjects: function (adapter, fileObjectList, callback) {
        ...
        function processFile(adapter, err, objectList) {
            ...
            _.each(groupAddresses, function (groupAddress) {
                ...
                if (dpt == "") {
                    const text = `Ignoring ${fullPath} because no DPT is assigned to ${groupAddress.getAttribute("Address")}. Most likely this DPT is not connected to a KO in ETS`;
    ...
    findStatusGAs(adapter, gaList, callback) {
        let count = 0;
        const regex = RegExp(adapter.config.statusRegex); //"stat(e|us)|r\\u00fcckmeldung\\s|\\svalue"
        for (const statusElement of gaList) {
            if (regex.test(statusElement.toLowerCase())) {
                //for all status gas
                const reducedStatusElement = statusElement.toLowerCase().replace(regex, "");
                const nonStatusElement = Array.from(gaList).reduce((previousValue, currentValue) => {
                    ...
                    if (regex.test(currentValue.toLowerCase())) {
                        //found another status, skip
                        return previousValue;
                    }
                    //reduce the list to the best match
                    if (similarity(currentValue.toLowerCase(), reducedStatusElement) > similarity(previousValue.toLowerCase(), reducedStatusElement)) {
                        return currentValue;
                    } else {
                        return previousValue;
                    }
                ...
                if (similarity(expression.replace(" ", ""), maxElement.toLowerCase().replace(" ", "")) > adapter.config.regexSimil) {
                    //found a match that is not and status and good enough, generate alias object
                    createAlias(adapter, maxElement, element);
                    adapter.log.debug("create aliases: found match " + nonStatusElement + " and " + statusElement);
                    count++;
                }
            }
        }
