    ...
    generateIoBObjects: function (adapter, fileObjectList, callback) {
        ...
        function processFile(adapter, err, objectList) {
            ...
            _.each(groupAddresses, function (groupAddress) {
                ...
                if (dpt == "") {
                    const text = `Ignoring ${fullPath} because no DPT is assigned to ${groupAddress.getAttribute("Address")}. The GA is not connected to a KO in ETS`;
                ...
                if (dpt.indexOf(".") === -1) {
                    const text = `${fullPath} ${groupAddress.getAttribute("Address")} has only set a base DPT in ETS. No unit information for this GA available.`;
    ...
    findStatusGAs(adapter, gaList, callback) {
        ...
        for (const statusElement of gaList) {
            if (regex.test(statusElement.toLowerCase())) {
                ...
                if (similarity(reducedStatusElement.replace(" ", ""), nonStatusElement.toLowerCase().replace(" ", "")) > adapter.config.regexSimil) {
