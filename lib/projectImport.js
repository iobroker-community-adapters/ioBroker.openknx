            ...
            if (regex.test(statusElement)) {
                ...
                if (similarity(a, b) > adapter.config.regexSimil) {
                    ...
                    if (gaList.getDataById(statusElement).native.dpt != gaList.getDataById(nonStatusElement).native.dpt) {
                        const text = "create aliases: " + gaList.getDataById(statusElement).native.dpt + " does not match " + gaList.getDataById(nonStatusElement).native.dpt + ", consider defining a conversion function in the alias object";
                        adapter.log.warn(text);
                        err += text + "<br/>";
                    }
                }
