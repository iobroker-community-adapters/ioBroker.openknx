...
function detect(ip, device, options, callback) {
    ...
    addresses.forEach(ip => {
        udpScan(mcast, mport, ip, undefined, openknxTimeout, false, (err, msg) => {
            if (!msg) {
                ...
                if (
                    ...
                    service_identifier_1 != 0x2 ||
                    service_identifier_2 != 0x2
                ) {
                    //Unknown KNX header format
                    return;
                }
...
function udpScan(probeAddress, probePort, listenAddress, listenPort, timeout, onlyOneResult, callback) {
    ...
    udpSocket.on("message", (message, remote) => {
        // Discovery response received from ${remote.address}:${remote.port}
        if (onlyOneResult) {
            clearTimeout(probeTimeout);
            try {
                udpSocket.close();
    ...
    udpSocket.on("listening", () => {
        ...
        const bytes = [
            ...
            0x02,
            0x01, //Service Identifier: KNX Search Request
            0x00,
            0x0e, //Total length
 ...
 *
 * @alias getOwnAddress
 * @returns {string[]} own ip address of the interface which we can use to communicate with desired device
 */
